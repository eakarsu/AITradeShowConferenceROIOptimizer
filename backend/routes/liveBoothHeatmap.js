...
// Real-time booth traffic heatmapping for on-the-spot optimization
// Feature: live-booth-heatmap
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Real-time booth traffic heatmapping for on-the-spot optimization.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'live-booth-heatmap', payload, result);
    res.json({ success: true, feature: 'live-booth-heatmap', ...result });
  } catch (err) {
    console.error('live-booth-heatmap analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Real-time booth traffic heatmapping for on-the-spot optimization. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'live-booth-heatmap', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'live-booth-heatmap', title: 'Real-time booth traffic heatmapping for on-the-spot optimization', project: 'AITradeShowConferenceROIOptimizer' });
});
