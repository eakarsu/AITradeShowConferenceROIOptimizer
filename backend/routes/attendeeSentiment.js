...
// Attendee sentiment tracking via post-event surveys to predict close probability
// Feature: attendee-sentiment
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITradeShowConferenceROIOptimizer" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Attendee sentiment tracking via post-event surveys to predict close probability.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'attendee-sentiment', payload, result);
    res.json({ success: true, feature: 'attendee-sentiment', ...result });
  } catch (err) {
    console.error('attendee-sentiment analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITradeShowConferenceROIOptimizer". Return strict JSON only.',
      `Feature: Attendee sentiment tracking via post-event surveys to predict close probability. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'attendee-sentiment', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'attendee-sentiment', title: 'Attendee sentiment tracking via post-event surveys to predict close probability', project: 'AITradeShowConferenceROIOptimizer' });
});
