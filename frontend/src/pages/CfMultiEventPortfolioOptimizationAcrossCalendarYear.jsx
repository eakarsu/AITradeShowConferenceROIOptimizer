...
export default function CfMultiEventPortfolioOptimizationAcrossCalendarYear() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-event-portfolio-optimization-across-calendar-year/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-event-portfolio-optimization-across-calendar-year', project: 'AITradeShowConferenceROIOptimizer' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-event-portfolio-optimization-across-calendar-year/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-event portfolio optimization across calendar year</h1>
