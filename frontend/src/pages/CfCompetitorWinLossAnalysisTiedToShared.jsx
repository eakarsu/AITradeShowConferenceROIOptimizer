...
export default function CfCompetitorWinLossAnalysisTiedToShared() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-competitor-win-loss-analysis-tied-to-shared-events/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-competitor-win-loss-analysis-tied-to-shared-events', project: 'AITradeShowConferenceROIOptimizer' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-competitor-win-loss-analysis-tied-to-shared-events/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Competitor win/loss analysis tied to shared events</h1>
