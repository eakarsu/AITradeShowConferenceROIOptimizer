...
export default function CfAccountBasedMarketingTargetingPersonalizingOutreachTo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-account-based-marketing-targeting-personalizing-outreach-to-high-value-attendees/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-account-based-marketing-targeting-personalizing-outreach-to-high-value-attendees', project: 'AITradeShowConferenceROIOptimizer' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-account-based-marketing-targeting-personalizing-outreach-to-high-value-attendees/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Account-based marketing targeting personalizing outreach to high-value attendees</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITradeShowConferenceROIOptimizer</p>
      </div>
