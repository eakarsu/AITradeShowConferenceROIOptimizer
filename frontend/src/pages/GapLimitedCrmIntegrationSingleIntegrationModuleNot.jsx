...
export default function GapLimitedCrmIntegrationSingleIntegrationModuleNot() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-crm-integration-single-integration-module-not-salesforce/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-crm-integration-single-integration-module-not-salesforce', project: 'AITradeShowConferenceROIOptimizer' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-crm-integration-single-integration-module-not-salesforce/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited CRM integration (single integration module — not Salesforce/HubSpot specific)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AITradeShowConferenceROIOptimizer</p>
      </div>
