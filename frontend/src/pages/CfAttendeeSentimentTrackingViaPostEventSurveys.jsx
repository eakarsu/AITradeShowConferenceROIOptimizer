...
export default function CfAttendeeSentimentTrackingViaPostEventSurveys() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-attendee-sentiment-tracking-via-post-event-surveys-to-predict/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-attendee-sentiment-tracking-via-post-event-surveys-to-predict', project: 'AITradeShowConferenceROIOptimizer' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-attendee-sentiment-tracking-via-post-event-surveys-to-predict/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Attendee sentiment tracking via post-event surveys to predict close probability</h1>
