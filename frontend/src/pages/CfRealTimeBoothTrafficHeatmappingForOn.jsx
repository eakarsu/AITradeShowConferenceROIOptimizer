...
export default function CfRealTimeBoothTrafficHeatmappingForOn() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-real-time-booth-traffic-heatmapping-for-on-the-spot-optimization/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-real-time-booth-traffic-heatmapping-for-on-the-spot-optimization', project: 'AITradeShowConferenceROIOptimizer' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-real-time-booth-traffic-heatmapping-for-on-the-spot-optimization/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Real-time booth traffic heatmapping for on-the-spot optimization</h1>
